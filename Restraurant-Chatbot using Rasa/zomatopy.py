...
class Zomato:
    ...
    def restaurant_advance_search(self, query="", latitude="", longitude="", cuisines="", option={'sort':'rating', 'order':'dsc'}, limit=5):
        """
        Takes either query, latitude and longitude or cuisine as input.
        Returns a list of Restaurant IDs.

        Fetch the results based on budget for 2 using the options
        options = {'mincft':300, 'maxcft':700, 'sort':'rating', 'order':'dsc'}

        mincft will fetch the restaurants in which the average cost for two persons is less than it
        maxcft will fetch the restaurants in which the average cost for two persons is more than it
        """
        cuisines = "%2C".join(cuisines.split(","))
        if str(limit).isalpha() == True:
            raise ValueError('LimitNotInteger')
        headers = {'Accept': 'application/json', 'user-key': self.user_key}
        r = (requests.get(base_url + "search?q=" + str(query) + "&count=" + str(limit) + "&lat=" + str(latitude) + "&lon=" + str(longitude) + "&cuisines=" + str(cuisines), params=option, headers=headers).content).decode("utf-8")
        return r#a = ast.literal_eval(r)
